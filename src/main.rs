...
fn shade(hit: &Intersect, light: &Light) -> Color {
...
fn cast_ray(
    ...
    objects
        ...
        .map_or(BACKGROUND, |hit| shade(&hit, light))
}
...
fn render(framebuffer: &mut Framebuffer, cube: &Cube, camera: &Camera, light: &Light) {
    ...
    for y in 0..framebuffer.height {
        for x in 0..framebuffer.width {
            ...
            let color = cast_ray(&eye, &world_ray, &objects, light);
...
fn main() {
    ...
    let mut window = Window::new(
        "Ray Tracer Cube - Flechas o WASD para orbitar - Esc para salir",
        WIDTH,
        HEIGHT,
...
mod tests {
    ...
    fn lambert_light_changes_with_surface_orientation() {
        ...
        assert!(shade(&lit, &light).to_hex() > shade(&away, &light).to_hex());
    }
}
